...
// Additional validation method for custom validation requirements
export const validateCustomHeroRules = (req: Request, res: Response, next: NextFunction) => {
  // Example of additional validation rules
  const { customField } = req.body;

  if (customField && customField.length > 100) {
    return res.status(400).json({
      error: 'Custom field exceeds maximum length',
      details: 'Custom field must not exceed 100 characters'
    });
  }
